...
#define STRING_ERROR_OVERLAPPING_MEMORY_REGIONS \
	"strings have overlapping memory regions"
...
Error string_copy(String* string, const String* source) {
	...
	if (string->data != NULL) {
		ASSERT_ERROR(source->data < string->data ||
			source->data >= string->data + string->length,
			STRING_ERROR_OVERLAPPING_MEMORY_REGIONS);
	}
...
Error string_copy_raw(String* string, const char* source) {
	...
	if (string->data != NULL) {
		ASSERT_ERROR(source < string->data ||
			source >= string->data + string->length,
			STRING_ERROR_OVERLAPPING_MEMORY_REGIONS);
	}
...
Error string_append(String* string, const String* another) {
	...
	if (string != another && string->data != NULL) {
		ASSERT_ERROR(another->data < string->data ||
			another->data >= string->data + string->length,
			STRING_ERROR_OVERLAPPING_MEMORY_REGIONS);
	}
...
Error string_append_raw(String* string, const char* another) {
	...
	if (string->data != NULL) {
		ASSERT_ERROR(another < string->data ||
			another >= string->data + string->length,
			STRING_ERROR_OVERLAPPING_MEMORY_REGIONS);
	}
